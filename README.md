...
3. **设规则**（右侧「规则」页）——如「视力差学生必须在前 2 排」「优等生与需关注学生同桌」。
   不确定该挑哪一条时，点「添加规则 → 自定义规则（向导）…」：按「谁 → 必须 / 尽量 → 怎么样」
   拼一句话，向导会挑好对应的规则种类，并当场显示这条规则的中文描述。
...
* 菜单 **文件 → 下载名单导入模板…**，或左侧学生面板的 **更多名单操作 → 下载名单导入模板…**；
...
（3 组×6 行×2 列、4 组×5 行×2 列、2 组×6 行×3 列、单排 1×8、单列 8×1 ……），
可以按格式自行增删。
...
| 编号 | 功能 | 说明 |
|---|---|---|
| F1 | 教室布局编辑 | 1~6 个分组，每组独立行数（1~12）/ 列数（1~8）/ 组名 / 组间距；**单排 / 单列编辑**（模板一键套用，或「±一排 / ±一列」逐排逐列加减）；讲台方向、座位卡片三档尺寸、是否显示组标题（三者都属于布局，随项目保存）；左侧参数 + 右侧实时预览 + 形状提示；高级选项默认折叠 |
| F2 | 学生名单管理 | 学号 / 姓名 / 性别 / 标签 / 数值属性 / 备注；`QTableView` + `QAbstractTableModel`；自然序与拼音排序；实时搜索与标签筛选；**拖学生到座位完成排位 / 从座位拖回名单取消入座**；Excel 导入（字段映射 + 错误行高亮 + 可跳过继续）；**一键生成名单导入模板**；名单导出 |
| F3 | 标签与选区 | 12 色标签库（自动分配未用色）；批量打标签；选区框选 / 按组 / 按行 / 按列创建，命名保存复用；选区批量清空 / 空置 / 分配 |
| F4 | 智能排位 | 13 条硬约束 + 14 条软约束；自定义规则向导（按「谁 → 要求 → 怎么样」拼装）；随机重启 + 爬山法；排位报告（硬约束明细 + 软约束得分）；「换一批」「锁定座位再排位」 |
...
| F9 | 撤销 / 重做 | 快照式，默认 50 步，跨操作类型一致；`Ctrl+Z` / `Ctrl+Y`（或 `Ctrl+Shift+Z`）；工具栏按钮 tooltip 显示操作名称 |

### 规则一览

规则分**硬约束**（必须满足，违反处红色高亮）与**软约束**（加权评分，尽量满足）两类。

表中的「对象 A / 对象 B」既可以选标签（作用于这类学生），也可以指定具体学生；
两者都填时按学生算。两端填同一个标签是合法的，例如「视力差」配「视力差」表示
"同类不得相邻 / 不得同桌"。「相邻」一律指**组内**上下左右，越过过道的两个座位不算相邻。

不确定该挑哪一条时，用「添加规则 → 自定义规则（向导）…」。
...
| 固定座位 | 学生 + 座位 |
| 禁止相邻 | 学生 A + 学生 B |
| 禁止相邻（标签或学生） | 对象 A + 对象 B |
| 必须相邻 | 对象 A + 对象 B |
| 必须同桌 | 对象 A + 对象 B |
| 禁止同桌 | 对象 A + 对象 B |
| 必须同组 / 同排 | 对象 A + 对象 B + 同组 / 同排 |
| 四周留空 | 标签 / 学生 + 可以相邻的标签（可选） |
| 区域限制 | 标签 / 学生 + 选区 |
| 排除区域 | 标签 / 学生 + 选区 |
| 前排必需 | 标签 / 学生 + 前 N 排 |
| 每组人数上限 | 每组最多 N 人 |
| 按顺序排座 | 按学号或数值属性 + 按排 / 按组 |
...
| 靠前偏好 | 标签 / 学生 |
| 靠后偏好 | 标签 / 学生 |
| 靠过道 / 居中偏好 | 标签 / 学生 + 靠左 / 靠右 / 两侧 / 居中 |
| 两人尽量靠近 | 对象 A + 对象 B |
| 各组人数均衡 | — |
| 各组男女均衡 | — |

**「每组人数上限」与「各组人数均衡」管的是"每组占几个座位"**，所以它们在排位开始前
挑座位的那一步就生效（上限装不下全班时会直接提示，而不是排出个超员的结果）。
...
鼠标：名单里选中**未分配**的学生 → 点击空座位即入座；从名单拖学生到座位；
**从座位拖回左侧名单 = 取消入座**（学生回到未分配列表）；
从座位拖到另一个座位即交换；在座位表空白处拖拽框选、`Ctrl+点击` 增减座位；
右键座位弹出「清空 / 空置 / 锁定」菜单；悬停 0.4 秒显示学生完整信息。

> 已入座的学生不会被「点一下空座位」搬走（点它只会提示当前座位），换座请直接拖动座位卡片；
> 批量放人用「编辑 → 把选中学生放入空座位」。
...
学生总数**基本无关**，只随**开启的规则条数**增长：60 座位实测 12 条规则约
**0.08ms/次**、27 条全开约 **0.35ms/次**（CPU 时间，一次交换要评估前后两次）。

**维护要点**：「增量打分的差 == 全量打分的差」是这套优化的正确性前提
（推导见 `RuleEngine.objective` 的 docstring）。每条规则的影响半径登记在
`RuleSpec.scope` 上（0 只看自己座位、1 还要看四邻域、2 还要看整组），
**登记小了这条不变量就会破**：增量评估与全量评估悄悄分叉，求解器仍然跑，
只是结果不再等于报表里显示的那个分数。引擎启动时会校验没有漏登记；
`tests/invariant_probe.py`（本机自检，不入库）对每个规则种类跑随机交换来盯住它。

> 说明：PRD 里「3 秒内完成 200+ 次重启」是估算值。本实现把「座位占用集合」
> 在整个搜索过程中固定，用**同一批座位上的学生排列**做爬山，因此每次重启都
> 是彻底重新洗牌，收敛质量比盲目提高重启次数更重要。实测 60 座位 / 3 秒
> 可完成约 16000 次交换评估（12 条规则），软约束满足度 80~95%、硬约束 0 违反。
...
* **布局快速模板 JSON 读取 `name` / `groups` / `rows` / `cols`**；
  `podium_side`、`card_size`、`gap_after` 暂不读取，套用模板时座位卡片尺寸按形状
  自动选（一行 ≥ 5 列用「小」，否则用「中」，可在「高级选项 → 座位尺寸」里改），
  讲台方向与组间距沿用对话框里的默认值（详见 `resources/templates/README.md`）。
* **座位尺寸 / 组标题属于项目本身**：打开旧项目时以项目里的设置为准，
  本机上次的选择只用来初始化全新项目。
* **窗口尺寸按屏幕可用区域收口**：主窗口、布局编辑器与各对话框都不会超过屏幕
  可用范围（高 DPI 缩放 150% / 200% 时尤其重要）；菜单也拆到能一屏放下——
  「添加规则」只留向导，27 条规则分成「硬约束 / 软约束」两个子菜单。
* **启动即最大化**：每次打开程序都直接最大化主窗口（座位表越宽越好用）；
  点「还原」后的尺寸仍然是上次退出时的样子。
